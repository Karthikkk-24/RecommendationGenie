...
function isAuthError(error: unknown): boolean {
  ...
  return /not authenticated|unauthenticated|session expired/i.test(error.message);
}
  ...
  if (media.isError || !media.data) {
    return (
      <main className="mx-auto max-w-lg space-y-4 px-6 py-16">
        ...
        <Button href="/app/search">Back to search</Button>
      </main>
    );
  }

  const item = media.data;
  const needsLogin = match.isError && isAuthError(match.error);
  ...
  return (
    <main className="mx-auto grid max-w-6xl gap-8 px-6 py-10 md:grid-cols-[280px_1fr]">
      ...
      <div>
        ...
        {match.data ? (
          <Card className="mt-6">
            ...
            <p className="mt-2 text-xs text-[var(--muted)]">
              ...
              {Math.round(match.data.scores.quality * 100)}% · Novelty {Math.round(match.data.scores.novelty * 100)}%
            </p>
          </Card>
        ) : null}
        ...
        {needsLogin ? (
          <Card className="mt-6">
            <p className="text-sm text-[var(--muted)]">Sign in to see your Genie match and save this title.</p>
            <div className="mt-3">
              <Button href={`/login?next=${encodeURIComponent(`/media/${item.id}`)}`}>Log in</Button>
            </div>
          </Card>
        ) : null}
